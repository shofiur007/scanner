    ...
    for entry in foundEntryPoints:
        if(entry.isDangerous() == True):
            ...
            logger.info("[Ending] Entry point -> " + entry.name +
                        " with dangerous: " + str(entry.isDangerous()))
...
def insideSink(line, sink):
    ...
    if (sinkIndex == -1):
        logger.error("[ERROR] no sink:" + sink.name +
                     " was found! What am I doing here?")
        ...
        if(entryPoints == []):
            ...
            if(len(entryPoints) == 1):
                logger.debug("FOUND DANGEROUS ENTRY_POINT!!! ->" +
                             entryPoints[0].name)
...
def insideValidation(line, val):
    ...
    if (valIndex == -1):
        logger.error("[ERROR] no validation:" + val.name +
                     " was found! What am I doing here?")
        ...
        for line in lines:
            if line.rstrip().endswith(';'):
                if(temp_line != ''):
                    ...
                    logger.debug(
                        "Multiple lines added to file: \"" + temp_line + "\"")
                    temp_line = ''
